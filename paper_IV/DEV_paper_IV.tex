...
ad-hoc interpolation function~\cite{DEV_paperI,DEV_paperII}. Paper~I
...
\mathrm{eV}/c^2$~\cite{DEV_paperII}.

Paper~III of the series~\cite{DEV_paperIII} took a different route:
...
discusses its mass-profile independence. Section~\ref{sec:numerical}
...
The DEV Lagrangian density is~\cite{DEV_paperI}
\begin{equation}
...
is directly comparable to the regression exponent of Paper~III.
The relation between $\alpha$ and $\gamma_\mathrm{eff}$ was
established numerically in Paper~III~\cite{DEV_paperIII}, where a
power-law regression of
$S_\mathrm{eff}(r) \equiv \nabla^2[(\eta-1)\Psi]$ over the
deep-MOND window yielded $\alpha = -1.56\pm0.02$ for the
point-source profile. The present paper derives
$\gamma_\mathrm{eff}$ analytically from the Green function
$G(r)\sim r^{-\gamma_\mathrm{eff}}$ of the quasilinear
operator~\eqref{eq:L_operator}; the empirical mapping
\begin{equation}
  \alpha = -(1 + 2\gamma_\mathrm{eff})
  \label{eq:alpha_gamma}
\end{equation}
then provides the closed-form bridge between the two papers.
A first-principles derivation of~\eqref{eq:alpha_gamma} from the
full DEV slip formula
$\eta - 1 = \tfrac{2}{3}\beta[(g/a_0)(1+g/a_0)]^{-1/2}$ is left
for future work.
The numerical validation of this WKB approximation is presented
in Section~\ref{sec:numerical}: Table~\ref{tab:profiles} shows
agreement within 5\% for $\gamma_\mathrm{eff}$ and 10\% for
$\alpha$ across all four mass profiles.

\paragraph*{Honest analytical status of $\alpha$.}
The leading-order expansion of
$\mathcal{S}_{\rm eff}(r) = \nabla^2[(\eta-1)\Psi]$ for a point
source in the deep-MOND limit reproduces the power-law exponent
$\alpha = -3/2$ exactly, as can be verified by direct
substitution of $\eta - 1 \propto \sqrt{r}$ and
$\Psi \propto \ln r$ (verified symbolically via \textsc{SymPy}).
The $4\%$ residual between this analytical leading value and the
numerical $\alpha = -1.56 \pm 0.02$ of Paper~III
\cite{DEV_paperIII} is attributable to
(i)~sub-leading corrections in $\eta - 1$ of order
$x = r_{\rm MOND}/r$,
(ii)~deviations of the exact AQUAL potential from its
asymptotic logarithm near $r \simeq r_{\rm MOND}$, and
(iii)~finite-window effects in the power-law regression. A
complete closed-form derivation that pins
$\alpha = -1.56$ to better than $1\sigma$ remains open and is
...
Paper~III and in Section~\ref{sec:numerical} arises because the
...
\eqref{eq:Lop} for the four mass profiles of Table~\ref{tab:profiles}.
...
relation \eqref{eq:alpha_gamma}, $\alpha = -(1+2\gamma_{\rm eff})$, with
...
relation \eqref{eq:alpha_gamma} holds only in an angle-averaged sense.
...
\eqref{eq:alpha_gamma} for any class of profiles would falsify the
...
Executing the equivalent script interactively yields:
\begin{verbatim}
>>> sp.simplify( d/dr[r^{2-s} dG/dr] )
0
>>> sp.limit( (r^{s-1}-1)/(s-1), s, 1 )
log(r)
\end{verbatim}
\noindent
The first output confirms that $G = Ar^{s-1}+B$ is an exact solution
of $(r^{2-s}G')'=0$ for arbitrary real $s$. The second confirms the
logarithmic limit $G \to A\ln r + B$ as $s\to 1$.

\begin{thebibliography}{9}

\bibitem{DEV_paperI}
M.~A.\ Mendes, \emph{Vacuum Excitation Dynamics: A
Scalar-Vector-Tensor Field Theory of Dark Matter as a Phase
Transition of the Quantum Vacuum} (2026), Physical Review D,
manuscript \texttt{DS14085} (under review). Zenodo preprint:
\url{https://doi.org/10.5281/zenodo.20137654}.

\bibitem{DEV_paperII}
M.~A.\ Mendes, \emph{Stability, Scale Constraints, and Parameter
Robustness of Vacuum Excitation Dynamics} (2026), Physical Review
D, manuscript \texttt{DS14151} (under review). Zenodo preprint:
\url{https://doi.org/10.5281/zenodo.20137528}.

\bibitem{DEV_paperIII}
M.~A.\ Mendes, \emph{Non-Local Gravitational Slip in Vacuum
Excitation Dynamics: Extended-Source Derivation} (2026), Physical
Review D, manuscript \texttt{DS14188} (under review). Zenodo
preprint: \url{https://doi.org/10.5281/zenodo.20137588}.
