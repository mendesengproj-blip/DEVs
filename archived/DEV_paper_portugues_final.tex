...
por $\mu_{\rm eff} > 1$.
A teoria prediz deslizamento gravitacional de $2$--$10\%$
...
\begin{figure}[h]
  ...
  \caption{A função de interpolação DEV $\mu(x)=x/\sqrt{1+x^2}$
           derivada do Lagrangeano DBI. As linhas tracejadas mostram
           os limites assintóticos: MOND profundo ($\mu\to x$, azul)
           e Newton ($\mu\to1$, vermelho).}
  \label{fig:mu}
\end{figure}
...
\begin{figure}[h]
  ...
  \caption{Relação de Aceleração Radial para a amostra SPARC ($n=167$).
           A linha sólida é a previsão DEV sem parâmetros globais livres.}
  \label{fig:rar}
\end{figure}
...
\begin{figure*}[t]
  ...
  \caption{Curvas de rotação representativas abrangendo quatro décadas
           em massa bariônica. Preto sólido: previsão DEV; vermelho
           tracejado: expectativa newtoniana; pontos azuis: observações SPARC.}
  \label{fig:rotcurves}
\end{figure*}
...
\begin{figure}[h]
  ...
  \caption{Calibração do parâmetro de acoplamento $\beta$ a partir de
           vínculos observacionais publicados de deslizamento gravitacional.
           A linha sólida é a previsão DEV com $\beta_{\rm best}=0{,}0075$,
           $\alpha=2/3$. Estrelas douradas marcam as seis UDGs da Tabela III.}
  \label{fig:beta}
\end{figure}
...
\begin{ruledtabular}
\begin{tabular}{lcccc}
Sistema & $g/\aO$ & $\eta-1$ (\%) & Euclid? \\
\hline
NGC1052-DF2 & 0,048 & 2,2 & Sim \\
NGC1052-DF4 & 0,068 & 1,8 & Sim \\
DF44        & 0,016 & 3,9 & Sim \\
DGSAT-I     & 0,0025 & 9,9 & Sim \\
VCC1287     & 0,017  & 3,8 & Sim \\
DF17        & 0,006  & 6,4 & Sim \\
\end{tabular}
\end{ruledtabular}
\end{table}
...
\begin{figure}[h]
  ...
  \caption{Deslizamento gravitacional $\eta-1$ em função de $g/\aO$
           para três valores de $\beta$. A banda verde marca a
           sensibilidade do Euclid (${\sim}1$--$5\%$).}
  \label{fig:slip}
\end{figure}
...
em empilhamento chega a ${\sim}8\sigma$.

\begin{figure}[h]
  \centering
  \includegraphics[width=\columnwidth]{figures/fig5_fisher_forecast.png}
  \caption{\textit{Esquerda:} Razão sinal-ruído para detecção de
           $\beta=0{,}01$ em função do número de UDGs com medições
           de lensing fraco. \textit{Direita:} Deslizamento gravitacional
           previsto para as seis UDGs conhecidas.}
  \label{fig:fisher}
\end{figure}
...
simultaneamente predizer $\eta - 1 \sim 10\%$ em DGSAT-I enquanto
