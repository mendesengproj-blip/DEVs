...
This expression is derived as the Green's function solution for a
point source in the deep-MOND limit ($g \ll \aO$). The
regularization factor $(1+g/\aO)$ in the denominator is an
interpolation ansatz, consistent with the $\nu$-function of
Eq.~(\ref{eq:nu}), that extends the formula to the Newtonian regime
($g \gg \aO$, where $\eta - 1 \to 0$ correctly). For extended mass
distributions, the full convolution with the Green's function is
required; the point-source approximation is accurate when the
relevant scales satisfy $r \gg r_{\rm MOND} \equiv \sqrt{GM/\aO}$,
which holds for all UDG systems in Table~\ref{tab:udg}.
