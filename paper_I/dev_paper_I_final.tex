...
statistically indistinguishable from \LCDM{} ($p = 0.64$, equivalent
significance ${\approx}0.5\sigma$); the sign of the preference reverses
for $\sigma_8\gtrsim 0.84$, so discrimination between the two models
awaits improved $f\sigma_8$ measurements (e.g.\ Euclid spectroscopic
survey).
The theory predicts gravitational slip of $2$--$7\%$ in
...
discussed in detail in Paper~II of this series.
...
discs $\alpha \simeq 1.0$. We verified that varying $\alpha$ across
$[0.5,\,1.0]$ leaves the SPARC fit $\chi^2_\nu$ bit-identical for
all tested galaxies (NGC\,2403, NGC\,3198, UGC\,02953; identity
verified at the \textsc{ieee-754} double-precision level), as
$\alpha$ enters the theory only through the gravitational slip and
not through the modified Poisson equation for the rotation curve.
There is therefore exactly zero sensitivity of the SPARC $\chi^2$
to the geometric factor $\alpha$; $\alpha$ propagates only into the
slip prediction $\eta(g)$ tested in UDGs (Sec.~\ref{sec:UDG}).
...
\begin{table}[h]
\caption{Head-to-head comparison of DEV, simple-$\mu$ MOND
($\mu_s = x/(1+x)$), and NFW with a Dutton \& Macci\`o (2014)
concentration--mass prior, on 167 SPARC galaxies using an
identical fitting pipeline and priors
$\Upsilon_{\rm disk}\in[0.3,\,5.0]$,
$\Upsilon_{\rm bul}\in[0.3,\,3.0]$.}
\label{tab:comparison}
\begin{ruledtabular}
\begin{tabular}{lccccc}
Model & $k$/gal & $\chi^2_{\nu,\rm med}$ & $f_{<1.5}$ & AIC & BIC \\
\hline
DEV (this work)  & 2 & \textbf{1.30} & $56\%$ & 9279 & 11322 \\
MOND ($\mu_s$)   & 2 & 1.58          & $47\%$ & 11060 & 13103 \\
NFW + DM14       & 4 & 0.95          & $67\%$ & \textbf{4919} & \textbf{9005} \\
\end{tabular}
\end{ruledtabular}
\end{table}

DEV achieves $\chi^2_{\nu,\rm med} = 1.30$ with two
mass-to-light ratios as the only free parameters,
outperforming simple-$\mu$ MOND ($\chi^2_{\nu,\rm med} = 1.58$;
$\Delta{\rm AIC} = -1781$) in the same pipeline, confirming
that the DEV $\nu$-function is a genuine improvement over the
standard MOND interpolation. An NFW halo with a Dutton \&
Macci\`o (2014) concentration--mass prior achieves
$\chi^2_{\nu,\rm med} = 0.95$ at the cost of two additional
halo parameters per galaxy; we make no claim of fit-quality
parity with the cold-dark-matter parametrization, only that
DEV provides a competitive prediction with no per-galaxy halo
freedom and no dark matter component.

The RAR predicted by DEV exhibits an intrinsic scatter of
$0.095$~dex across the 167-galaxy SPARC sample (after
subtracting observational error in quadrature), below the
McGaugh, Lelli \& Schombert~(2016)~\cite{McGaugh2016}
empirical bound of $0.13$~dex and superior to the $0.108$~dex
obtained under simple-$\mu$ MOND in the same pipeline. This
is achieved with the stellar mass-to-light ratios as the only
per-galaxy free parameters, with no halo tuning.
...
$\sigma_8$: a numerical scan over $\sigma_8\in[0.78,\,0.92]$
locates the crossover at $\sigma_8\approx 0.837$, beyond which
\LCDM{} is preferred. A joint fit over $\sigma_8$ is deferred
to the full Boltzmann analysis.

For the combined $f\sigma_8$ dataset ($N=7$), DEV and \LCDM{}
are statistically indistinguishable: $|\Delta\chi^2|=0.22$
corresponds to $p=0.64$ (equivalent significance $\approx 0.5\sigma$)
when $\beta$ is treated as a single additional parameter. The
mild amplification of structure growth by $\mu_{\rm eff}>1$ in
the Newton regime of DEV is therefore consistent with current
observations but not yet detectable; improved $f\sigma_8$
precision of order $30\%$ (e.g.\ Euclid spectroscopic survey)
is required to discriminate the two models.
...
\begin{figure}[h]
  ...
  \caption{Calibration of the coupling parameter $\beta$ from
           ...
           the shaded band shows the formal $1\sigma$ range
           $\beta\in[0.0015,\,0.0134]$ from the five-point fit
           (inflated by $\chi^2_{\rm red}=0.034$). The defensible
           rescaled interval adopted in this work is
           $\beta=0.0070\,[0.0055,\,0.0085]$
           (Eq.~\ref{eq:beta_calib}).
...
of the point-source approximation. All six systems have
$\epsilon > 1$, placing them in the extended-source regime;
the point-source formula provides a lower bound on $\eta-1$,
with corrections quantified in Paper~III.}
\label{tab:udg}
\begin{ruledtabular}
\begin{tabular}{lccccc}
System & $g/\aO$ & $\eta-1$ (\%) & $r_{\rm eff}/r_{\rm MOND}$ & Euclid? \\
\hline
NGC1052-DF2 & 0.048 & 2.23 & 4.56 & Yes \\
NGC1052-DF4 & 0.044 & 2.35 & 4.79 & Yes \\
DF44        & 0.019 & 3.61 & 7.28 & Yes \\
DGSAT-I     & 0.016 & 3.95\,--\,6.7$^{b}$ & 7.96 & Yes \\
VCC1287     & 0.015 & 4.08 & 8.21 & Yes \\
DF17        & 0.019 & 3.60 & 7.26 & Yes \\
\end{tabular}
\end{ruledtabular}
{\small Values recomputed (Paper~III) using published
photometric $r_{\rm eff}$. The $\eta-1$ column uses
$x \equiv g_N/\aO$. All six systems lie in the deep-MOND
regime ($\epsilon \equiv r_{\rm eff}/r_{\rm MOND}>1$);
extended-source corrections of order unity are expected and
quantified in Paper~III.\\
$^{b}$ Point-source value (lower bound); the extended-source
correction derived in Paper~III of this series yields
$\eta-1 \simeq 6.7\%$.}
\end{table}
...
The point-source formula provides a lower bound for systems
with $\epsilon \gg 1$. For DGSAT-I ($\epsilon = 7.96$), the
extended-source correction derived in Paper~III yields
$\eta-1 \simeq 6.7\%$, strengthening its status as the
highest-priority Euclid target.
...
$\chi^2_{\rm min}/{\rm ndof} = 0.13/4$. The low
$\chi^2_{\rm red} = 0.034$ ($p = 0.998$) indicates that the
uncertainties of the five constraints are conservatively
large by a factor of approximately $5.5$, reflecting the
heterogeneous nature of these measurements and the absence
of a dedicated weak-lensing campaign targeting the
saturated-vacuum regime ($g \lesssim 0.05\,\aO$). The
unrescaled formal $1\sigma$ interval is therefore inflated
to $[0.0015,\,0.0134]$ and does not reflect the actual
agreement of the model with the data. Rescaling the
uncertainties to match $\chi^2_{\rm red} = 1$ and removing
the under-constrained Coma outer estimate (which is derived
from published velocity dispersions rather than a direct
$\eta$ measurement) yields the defensible interval
\begin{equation}
  \beta = 0.0070\,[0.0055,\,0.0085]\quad(1\sigma),
  \label{eq:beta_calib}
\end{equation}
with the central value shifted by only $6\%$ relative to the
five-point fit. The calibration is therefore robust to both
rescaling and to the removal of the weakest constraint, and we
adopt Eq.~\eqref{eq:beta_calib} in the gravitational-slip
predictions of Sec.~\ref{sec:UDG}.
...
\cite{vanDokkum2015}.
With $N=300$ UDGs and $\sigma_\eta=0.02$ (achievable via
tomographic weak-lensing stacking), the projected stack SNR
reaches ${\sim}31\sigma$, providing a highly significant test
of DEV within the Euclid survey lifetime
(Fig.~\ref{fig:fisher}).
...
predict $\eta - 1 \simeq 4\%$ in DGSAT-I while maintaining
...
statistically indistinguishable from \LCDM{} at current precision),
...
The Bullet Cluster operates deep in the Newtonian regime of
DEV ($g/\aO\approx 21$ at the core), so the gravitational slip
is suppressed ($\eta-1\sim 10^{-3}$) and the order-of-magnitude
estimate of Appendix~\ref{app:bullet} reproduces the observed
${\sim}200$~kpc offset through standard ram-pressure deceleration
of the gas, with a single phenomenological fraction
$f_{\rm decel}$ rather than as a DEV-specific prediction. DEV is
therefore consistent with the Bullet Cluster at the
order-of-magnitude level, as are MOND and TeVeS in their
standard formulations; a quantitative test that discriminates
the three frameworks requires fully non-linear simulation of the
coupled $\{\theta, A_\mu\}$ sector through a merger event, which
...
This estimate is order-of-magnitude only and is \emph{not} a
falsifiable DEV prediction. Writing the crossing time as
$t_{\rm cross} = d_{\rm sep}/v_{\rm rel}$ with
$d_{\rm sep}\approx 720$~kpc the post-pericenter sub-cluster
separation, the displacement formula simplifies algebraically to
\begin{equation}
  \Delta x = f_{\rm decel}\,v_{\rm rel}\,t_{\rm cross}
           = f_{\rm decel}\,d_{\rm sep},
  \label{eq:bullet_simplified}
\end{equation}
in which $v_{\rm rel}$ and $r_c$ cancel and no DEV parameter
($\beta$, $\alpha$, $\nu$) appears. Reproducing the observed
$\Delta x\approx 200$~kpc requires $f_{\rm decel}\approx 0.28$,
within the range $[0.1,\,0.5]$ inferred from hydrodynamical
simulations~\cite{SpringelFarrar2007}. The Bullet Cluster
therefore constrains $f_{\rm decel}$, not DEV; it operates
entirely in the Newtonian regime of the theory, so it neither
confirms nor falsifies the core prediction. A genuine
DEV-specific test at cluster scales would require fully
non-linear simulation of the coupled $\{\theta, A_\mu\}$
system through the merger, which we defer to future work. The
definitive falsification test remains the gravitational slip
in UDGs with $g \ll \aO$.
